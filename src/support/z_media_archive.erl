...
make_unique(Archive, Rootname, Extension) ->
    File = z_string:concat(filename:join([Archive, Rootname]), Extension),
    case filelib:is_file(File) of
        true ->
            make_unique(Archive, Rootname, Extension, 1);
        false -> 
            z_string:concat(filename:join([Rootname]), Extension)
    end.

make_unique(Archive, Rootname, Extension, Nr) ->
    File = z_string:concat(filename:join([Archive, z_string:concat(Rootname, [$-|integer_to_list(Nr)])]), Extension),
    case filelib:is_file(File) of
        true ->
            make_unique(Archive, Rootname, Extension, Nr+1);
        false -> 
            z_string:concat(z_string:concat(Rootname, [$-|integer_to_list(Nr)]), Extension)
    end.
